...
def calc_output(locale):
  ...
  #e.g. '<(ASSETS_DIR)/da.pak',
  return os.path.join(ASSETS_DIR, locale + '.pak')

def calc_inputs(locale):
  """Determine the files that need processing for the given locale."""
  inputs = []

  # SWE-TODO: Investigate if it needs to be pulled in.
  ...
  # SWE-TODO: Investigate if it needs to be pulled in.
  #e.g. '<(grit_out_dir)/chromium_strings_da.pak'
  # inputs.append(os.path.join(
      # GRIT_DIR, '%s_strings_%s.pak' % ('chromium', locale)))
...
def swe_repack_locales(locales):
...
def DoMain(argv):
  ...
  parser.add_option("-x", action="store", dest="assets_dir",
  ...
  if not (GRIT_DIR and ASSETS_DIR and SHARE_INT_DIR):
  ...
  return swe_repack_locales(locales)
